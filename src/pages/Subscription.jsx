...
import { AlertTriangle, CreditCard, LogOut, Store, Zap } from 'lucide-react';
...
function SubscriptionContent() {
  ...
  if (access.hasStoreAccess) return <Navigate to="/dashboard" replace />;
    ...
    <main style={{ minHeight: '100vh', background: 'radial-gradient(circle at 50% 10%, rgba(245, 158, 11, 0.16), transparent 34%), var(--bg-dark)', color: 'var(--text-primary)', padding: '32px 18px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <section className="card pf-card-premium" style={{ width: '100%', maxWidth: '720px', border: '1px solid rgba(245, 158, 11, 0.22)', boxShadow: '0 24px 80px rgba(0, 0, 0, 0.36), 0 0 70px rgba(245, 158, 11, 0.08)', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
          ...
          <div>
            <p style={{ color: 'var(--primary)', fontWeight: 800, fontSize: '12px', textTransform: 'uppercase', margin: 0 }}>Acesso vencido</p>
            <h1 style={{ margin: '4px 0 0', fontSize: 'clamp(28px, 5vw, 44px)', lineHeight: 1.05 }}>Regularize o acesso da sua loja</h1>
          </div>
        </div>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: '17px', lineHeight: 1.7, marginBottom: '22px' }}>
          {access.currentPeriodEnd ? `Seu acesso venceu em ${formatSubscriptionDate(access.currentPeriodEnd)}. ` : 'Seu período de acesso foi encerrado. '}
          O pagamento de <strong style={{ color: 'var(--primary)' }}>R$ 39,00</strong> libera mais 30 dias. Não existe renovação automática nem cartão salvo.
        </p>

        <div style={{ border: '1px solid var(--border)', borderRadius: '14px', padding: '14px', marginBottom: '24px' }}><Store size={18} color="var(--primary)" /><p style={{ margin: '10px 0 4px', fontWeight: 800 }}>{store?.nome || store?.name || 'Sua loja'}</p><span style={{ color: 'var(--text-secondary)', fontSize: '13px' }}>O acesso é liberado somente após a confirmação segura do pagamento.</span></div>
        ...
        <button type="button" onClick={handleCheckout} disabled={checkoutLoading} className="btn btn-primary" style={{ width: '100%', minHeight: '52px', marginBottom: '12px', gap: '10px', opacity: checkoutLoading ? 0.72 : 1, cursor: checkoutLoading ? 'not-allowed' : 'pointer' }}>
          <CreditCard size={18} />{checkoutLoading ? 'Criando checkout de teste...' : 'Regularizar acesso por R$ 39'}
        </button>
