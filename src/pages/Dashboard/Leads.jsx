  ...
  const handleDeleteLead = async (leadId) => {
    ...
    try {
      ...
      await createPersistentNotification({
        ...
        message: err.message || 'Não foi possível remover este lead.',
  ...
  const handleUpdateLeadStatus = async (lead, nextStatus) => {
    ...
    if (nextStatus === 'vendido') {
      if (!multiItemLead && availableStock !== null && availableStock <= 0) {
        notifyTransientWarning({
          ...
          message: 'Não há estoque suficiente para confirmar esta venda.',
          category: 'leads'
        });
    ...
    try {
      ...
      await createPersistentNotification({
        type: 'error',
        title: 'Não foi possível concluir',
        message: err.message || 'Não foi possível atualizar o status do lead.',
        category: 'operation_errors'
      });
      setFeedbackMessage({
        type: 'error',
